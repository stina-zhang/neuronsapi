...
class run_product_del(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        '''读测试用例sheet'''
        cls.data = GetData(fileName='/Users/stina/Desktop/apitest.xls', sheetName='product_del')
        cls.run_method = RunMethod()
        cls.com_util = CommonUtil()

    def test_run_product_del(self):
        res = None
        ...
        for i in range(1, rows_count):
            ...
            if self.com_util.is_equal_str(expect, res) == 1:
                ...
                logging.info("**********product_del test success***********")
                globalvar.pass_count.append(i)
            else:
                self.data.write_result(i, 'fail')
                logging.info("**********product_del test fail***********")
