...
class run_room_edit(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.roomid = requests.post(url=room_url, data=json.dumps(room_data), headers=cls.header).json()['result']
        '''读测试用例sheet'''
        cls.data = GetData(fileName='/Users/stina/Desktop/apitest.xls', sheetName='room_edit')
        cls.run_method = RunMethod()
        cls.com_util = CommonUtil()

    def test_run_room_edit(self):
        res = None
        ...
        for i in range(1, rows_count):
            ...
            request_data_1 = re.sub('sid', self.sceneid, request_data_old)
            request_data = re.sub('rid', self.roomid, request_data_1)
            ...
            if self.com_util.is_equal_str(expect, res) == 1:
                ...
                logging.info("**********room_edit test success***********")
                globalvar.pass_count.append(i)
            else:
                self.data.write_result(i, 'fail')
                logging.info("**********room_edit test fail***********")
