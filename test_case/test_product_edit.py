...
class run_product_edit(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        '''读测试用例sheet'''
        cls.data = GetData(fileName='/Users/stina/Desktop/apitest.xls', sheetName='product_edit')
        cls.run_method = RunMethod()
        cls.com_util = CommonUtil()

    def test_run_product_edit(self):
        res = None
        ...
        for i in range(1, rows_count):
            ...
            request_data_1 = re.sub('fid', self.firmid, request_data_old)
            request_data = re.sub('pid', self.productid, request_data_1)
            ...
            if self.com_util.is_equal_str(expect, res) == 1:
                ...
                logging.info("**********product_edit test success***********")
                globalvar.pass_count.append(i)
            else:
                self.data.write_result(i, 'fail')
                logging.info("**********product_edit test fail***********")
