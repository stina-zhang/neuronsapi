...
class run_firm_getAll(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        '''读测试用例sheet'''
        cls.data = GetData(fileName='/Users/stina/Desktop/apitest.xls', sheetName='firm_getAll')
        cls.run_method = RunMethod()
        cls.com_util = CommonUtil()
        '''获取token'''
        cls.header = OperationHeader().get_header()

    def test_run_firm_getAll(self):
        res = None
    ...
    def tearDownClass(cls) -> None:
        '''删除测试厂商'''
        sql_room = "SELECT id FROM firm WHERE flag=0 AND `code`>699"
